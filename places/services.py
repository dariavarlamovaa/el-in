...
def get_cities_and_places(city=None, month=None):
    ...
    places_data = Place.objects.filter(filter_data).values('city', 'name_eng', 'image_path', 'available_time').order_by(
        'city', 'name_eng')
    cities_and_places = defaultdict(list)
    for item in places_data:
        cities_and_places[item['city']].append(
            {'name_eng': item['name_eng'], 'image_path': item['image_path'], 'available_time': item['available_time']})
